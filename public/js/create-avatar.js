...
async function getUploadedAvatars(){
    try {
        ...
        for (const img of data) {
            const imgContainer = createEl("div", {className: "small-img shadow-md"})
            const imgEl = createEl("img", {className: "object-cover", src: img.image})
            const deleteBtn = createEl("button", {type: "button", className: "absolute top-0 right-0 bg-red-500 text-white tex-sm texxt-white px-1 rounded-md", textContent:"Delete"})
...
function initTopAvatarContainer(image = null) {
    ...
    const avatarImage = createImageSelector({fileName: "avatarImage", className: 'flex-1 parent'}, image);
...
function initAvatarContainer(image = null) {
    const container = createEl('form', { className: 'w-full bg-gray-300 p-2 max-w-full' });
    ...
    const addNewLocationBtn = createEl("button", {type: "button", textContent: "Add Location", className: "add-location-btn"})
    addNewLocationBtn.addEventListener("click", function(){
        bottomContainer.append(createImageSelector({inputName: `location[${index}]`, fileName: `location[${index}]`}))
        index++
    })
    ...
    if(image && image.locations.length > 0){
        for (let i = 0; i < image.locations.length; i++) {
            bottomContainer.append(createImageSelector({inputName: `location[${i}]`, fileName: `location[${i}]`}, image.locations[i]))
        }
    }
